...
def parse_url(url):
    ...
    * Password specified in userinfo and as query parameter is
    ...
    assert r.scheme in ("", "redis", "rediss", "unix"), ("Unsupported URI scheme", r.scheme)
    if r.scheme == "":
        return url, {}
    query = {}
...
def _parse_uri_options(params, path, password):
    ...
    assert db1 is None or db2 is None, ("Single DB value expected, got path and query", db1, db2)
    if db1 is not None:
        options["db"] = db1

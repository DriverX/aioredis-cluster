...
class SentinelPool:
    ...
    async def discover_master(self, service, timeout):
        ...
        for sentinel in pools:
            try:
                ...
                sentinel_logger.debug("DiscoverError(%r, %s): %r", sentinel, service, err)
    ...
    async def _get_masters_address(self, sentinel, service):
        ...
        state = await sentinel.execute(b"sentinel", b"master", service, encoding="utf-8")
        if not state:
    ...
    async def _get_slave_address(self, sentinel, service):
        ...
        slaves = await sentinel.execute(b"sentinel", b"slaves", service, encoding="utf-8")
        if not slaves:
            raise UnknownService()
...
class ManagedPool(ConnectionsPool):
    ...
    async def _create_new_connection(self, address):
        if address is _NON_DISCOVERED:
            ...
            sentinel_logger.debug("Discoverred new address %r for %s", conn.address, self._service)
            return conn
    ...
    def _drop_closed(self):
        ...
        if diff:
            ...
            sentinel_logger.debug("Dropped %d closed connnection(s); must rediscover", diff)

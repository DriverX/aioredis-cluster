from ..util import _NOTSET, _ScanIter, wait_convert
...
class SortedSetCommandsMixin:
    ...
    def zadd(self, key, score, member, *pairs, exist=None, changed=False, incr=False):
        ...
        if incr:
            if pairs:
                raise ValueError("only one score-element pair " "can be specified in this mode")
            args.append(b"INCR")
    ...
    def zremrangebylex(self, key, min=b"-", max=b"+", include_min=True, include_max=True):
    ...
    def zremrangebyscore(self, key, min=float("-inf"), max=float("inf"), *, exclude=None):

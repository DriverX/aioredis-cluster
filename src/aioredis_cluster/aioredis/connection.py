...
class RedisConnection(AbcConnection):
    ...
    def execute_pubsub(self, command, *channels):
        ...
        channels = [ch if isinstance(ch, AbcChannel) else mkchannel(ch) for ch in channels]
    ...
    def _end_transaction(self, obj, discard):
        assert self._in_transaction is not None, ("Connection is not in transaction", obj)
        self._transaction_error = None
        ...
        if obj is None:
            ...
            obj = [err] * len(recall)
        assert len(obj) == len(recall), ("Wrong number of result items in mutli-exec", obj, recall)
        res = []

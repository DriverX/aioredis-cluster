...
class Receiver:
    ...
    def patterns(self):
        ...
        return types.MappingProxyType({ch.name: ch for ch in self._refs.values() if ch.is_pattern})
    ...
    def iter(self, *, encoding=None, decoder=None):
        ...
        """
        return _IterHelper(
            self, is_active=lambda r: not r._queue.exhausted, encoding=encoding, decoder=decoder
        )
    ...
    def _put_nowait(self, data, *, sender):
        if self._queue.closed:
            logger.warning(
                "Pub/Sub listener message after stop:" " sender: %r, data: %r", sender, data
            )
            return
...
class ClosableQueue:
    ...
    async def get(self):
        await self.wait()
        assert self._queue or self._closed, ("Unexpected queue state", self._queue, self._closed)

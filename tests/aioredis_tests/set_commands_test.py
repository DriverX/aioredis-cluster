...
async def test_sdiffstore(redis):
    ...
    test_result = await redis.sdiffstore(b"not:" + destkey, b"not:" + key1, b"not:" + key2)
    assert test_result == 0
...
async def test_sinterstore(redis):
    ...
    test_result = await redis.sinterstore(b"not:" + destkey, b"not:" + key1, b"not:" + key2)
    assert test_result == 0
...
@redis_version(3, 2, 0, reason="The count argument in SPOP is available since redis>=3.2.0")
...
async def test_sunionstore(redis):
    ...
    test_result = await redis.sunionstore(b"not:" + destkey, b"not:" + key1, b"not:" + key2)
    assert test_result == 0

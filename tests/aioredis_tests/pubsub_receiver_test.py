...
async def test_subscriptions(create_connection, server):
    ...
    mpsc = Receiver()
    await sub.execute_pubsub("subscribe", mpsc.channel("channel:1"), mpsc.channel("channel:3"))
...
@pytest.mark.skipif(sys.version_info >= (3, 6), reason="json.loads accept bytes since Python 3.6")

...
async def test_xrange(redis, server_bin):
    ...
    messages = await redis.xrange(stream, start="0000000000000-0", stop="9900000000000-0")
    assert len(messages) == 3
...
async def test_xrevrange(redis, server_bin):
    ...
    messages = await redis.xrevrange(stream, start="9900000000000-0", stop="0000000000000-0")
    assert len(messages) == 3
...
async def test_xread_selection(redis, server_bin):
    ...
    messages = await redis.xread([stream], timeout=1, latest_ids=["0000000000000-0"], count=2)
    assert len(messages) == 2
...
async def test_xread_group_with_no_ack(redis):
    ...
    # read all pending messages
    messages = await redis.xread_group(
        "test_group", "test_consumer", ["test_stream"], timeout=1000, latest_ids=[">"], no_ack=True
    )
    assert len(messages) == 1
    stream, message_id, fields = messages[0]
    assert stream == b"test_stream"
...
async def test_xpending_get_messages(redis):
    ...
    (message_id, consumer_name, milliseconds_since_last_delivery, num_deliveries) = response[0]
...
async def test_xgroup_delconsumer(redis, create_redis, server):
    ...
    await redis.xread_group("test_group", "test_consumer", streams=["test_stream"], latest_ids=[0])

...
async def test_migrate_copy_replace(create_redis, server, serverB):
    ...
    ok = await redisA.migrate("localhost", serverB.tcp_address.port, "my-key", 0, 1000, copy=True)
    assert ok is True
...
@pytest.mark.skipif(sys.platform == "win32", reason="Seems to be unavailable in win32 build")
async def test_migrate_keys(create_redis, server, serverB):
    ...
    ok = await redisA.migrate_keys(
        "localhost", serverB.tcp_address.port, ("key1", "key2", "key3"), dest_db=0, timeout=1000
    )
    assert not ok
...
async def test_migrate__exceptions(redis, server, unused_port):
    ...
    with pytest.raises(ReplyError, match="IOERR .* timeout .*"):
        assert not (
            await redis.migrate("localhost", unused_port(), "my-key", dest_db=30, timeout=10)
        )
